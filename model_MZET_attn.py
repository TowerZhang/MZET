...
class Attn_MZETModel(BaseModel):
    def __init__(self, config):
        super(Attn_MZETModel, self).__init__(config)
    ...
    def add_memory_net_op(self):
        ...
        with tf.variable_scope('matrix_memory'):
            ...
            self.variable_summaries(matrix_mem)
        ...
        with tf.variable_scope('C_memory'):
            ...
            self.variable_summaries(C)

        with tf.name_scope('o_memory'):
            o = tf.matmul(pi_k, C)
            self.variable_summaries(o)

        with tf.name_scope('a'):
            sum_out = o + u
            self.a = self.linear_layer(sum_out, memory_size)
            self.variable_summaries(self.a)
    ...
    def add_logits_op(self):
        ...
        with tf.variable_scope('Logits'):
            ...
            self.logits_test = tf.nn.softmax(self.logits_test)
            self.variable_summaries(self.logits_test)

            self.logits_level1 = tf.matmul(self.a, self.sim_matrix_level1)
            self.logits_level1 = tf.nn.softmax(self.logits_level1)
            self.variable_summaries(self.logits_level1)

            self.logits_level2 = tf.matmul(self.a, self.sim_matrix_level2)
            self.logits_level2 = tf.nn.softmax(self.logits_level2)
            self.variable_summaries(self.logits_level2)

        with tf.name_scope('Label_pred'):
            self.label_pred_train = tf.argmax(self.logits, 1)
            self.variable_summaries(self.label_pred_train)
            self.label_pred_test = tf.argmax(self.logits_test, 1)
            self.variable_summaries(self.label_pred_test)
            self.labels_pred_level1 = tf.argmax(self.logits_level1, axis=1)
            self.labels_pred_level2 = tf.argmax(self.logits_level2, axis=1)
    ...
    def add_loss_op(self):
        ...
        max_margin_loss = tf.maximum(0.0, 0.1 - tf.multiply(IND, self.logits))
